  ...
  return (
    <div style={{ display:'flex', flexDirection:'column' as const, gap:28 }}>
      ...
      <div style={{ display:'flex', gap:12, flexWrap:'wrap' as const }}>
        {[
          { label:'REST API', color:REST, desc:'Infinia Management API — https://192.168.147.129/redapi/v1' },
          { label:'SSH + redcli', color:SSH, desc:'redcli CLI over SSH via paramiko — host 192.168.147.129:22' },
          { label:'S3 / boto3', color:S3, desc:'S3-compatible object store — https://192.168.147.129:8111 (or per-tenant vhost:8111)' },
        ].map(m => (
      ...
      <div style={{ padding:'14px 18px', borderRadius:10, border:'1px solid '+S3+'30', background:S3+'08', fontSize:13, color:'var(--text-muted)' }}>
        ...
        Default tenant (red) uses bare IP <code style={{ fontFamily:'monospace', fontSize:12 }}>https://192.168.147.129:8111</code>.
